...
Recently, RBM's have found applications in collaborative filtering, as illustrated in \cite{SMH07}. Our method is based on this paper, although there are some differences.

We first build an RBM with 5000 visible units $v^k_i$, for $k\in\{1,\dots,5\}$ and $i\in\{1,\dots,1000\}$, and $F$ hidden units $h_1,\dots, h_F$, where $F$ is a hyperparameter of the algorithm. Furthermore, for each hidden unit $h_j$ we add connections to every visible unit $v^k_i$. Each connection between $h_j$ and $v^k_i$ has an associated weight $W^k_{ij}$. For each $(i,j,k)$ triple, the initial value of $W^k_{ij}$ is sampled from a zero-mean normal distribution with small variance.

To train our RBM, we divide the users into small batches

Note that we train only a single RBM by feeding it users' data vectors in batches, and then updating the weights. In \cite{SMH07}, the presented method consists of building a different RBM for each user, which all share a global weight matrix. The observed ratings of each user are then used to train its associated RBM. After going through a batch, the global weight matrix is updated. In the end we obtain an RBM with the same structure as the one which we get through our method (i.e. with connections between every hidden unit and every visible unit). The prediction algorithm is the same in our method and in \cite{SMH07}.
