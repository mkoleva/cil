...
\begin{equation}
R_ij = 0.5*AvgUser[i] + 0.5*AvgFilm[j]
\label{eq:imputation}
\end{equation}
...
\subsubsection*{\textbf{SGD for SVD}} The main idea of the algorithm is to avoid the need of filling in the missing values in the k-rank approximation of the rating matrix. 
...
\subsubsection*{\textbf{RBM}}
stuff stuff
\\

\subsubsection*{\textbf{user-based kNN}} Find the top k most similar users using a Pearson distance metric. The rating is the weighted average of the top k user\'s rating for the film.
\\
\subsubsection*{\textbf{item-based kNN}}  Calculate the similarity between the films a user has already rated and the film in question. The predicted rating is then a weighted average of their ratings using the similarities as weights.
