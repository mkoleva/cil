...
We first build an RBM with 5000 visible units $v^k_i$, for $k\in\{1,\dots,5\}$ and $i\in\{1,\dots,1000\}$, and $L$ hidden units $h_1,\dots, h_L$, where $L$ is a hyper-parameter of the algorithm. Furthermore, for each hidden unit $h_j$ we add connections to every visible unit $v^k_i$. Each connection between $h_j$ and $v^k_i$ has an associated weight $W^k_{ij}$. Each hidden unit $h_j$ has an associated bias $b_j$ and each visible unit $v^k_i$ has an associated bias $b^k_i$. Finally, each unit $h_j$ has an associated binary state $s_j$ and each unit $v^k_i$ has an associated binary state $s^k_i$.
...
\[
	p(s^k_i=1|h)=\frac{\exp(b^k_i+\sum_{j=1}^L s_jW^k_{ij})}{\sum_{l=1}^{5}\exp(b^l_i+\sum_{j=1}^{L}s_jW^l_{ij})}
\]
and
\[
	p(s_j=1|V)=\frac{1}{1+\exp(-b_j-\sum_{i=1}^{1000}\sum_{k=1}^{5}s^k_iW^k_{ij})}
\]

To train our RBM, we divide the users into small batches

Note that we train only a single RBM by feeding it users' data vectors in batches, and then updating the weights. In \cite{SMH07}, the presented method consists of building a different RBM for each user, which all share the same weights and biases. The observed ratings of each user are then used to train its associated RBM. After going through a batch, the global weights and biases are updated. In the end we obtain an RBM with the same structure as the one which we get through our method (i.e. with connections between every hidden unit and every visible unit). 

The prediction algorithm is the same in our method and in \cite{SMH07} and \cite{L10}. Namely, we use the observed ratings of a user as the st
